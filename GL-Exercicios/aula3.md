...
Uma função é de ordem superior se recebe como argumento pelo menos uma função. Exemplos:
...
foldr (+) 0 [1..3] > 6  // começa pelo 0
foldl (*) 1 [1..3] > 6  // começam pelo []
par x = mod x 2 == 0
impar x = not . par x -- igual a not (par x)
```

- HOF permitem parametrizar funções passando funções e não só dados
